...
includes its data payload, and associated methods to write. Arrays marked with 
`ArrayKind.DRAM_PAYLOAD` are skipped during this pass because they are modelled through the DRAM
request/response interface instead of the generic register array path. SRAM payload arrays
remain materialised so the simulator can load initialization files and implement the simple black
box that mirrors the Verilog backend.
