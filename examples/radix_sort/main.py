...
class MemUser(Module):
    ...
    def build(self, SM_reg: RegArray, radix_reg: RegArray, offset_reg: RegArray, addr_reg: RegArray, mem_pingpong_reg: RegArray):
        ...
        with Condition(SM_reg[0] == UInt(2)(1)):
            log("Stage 1: Read rdata=({:08x}) from memory addr_reg[0]=({:08x})", rdata, addr_reg[0]-UInt(addr_width)(1))
...
class MemImpl(Downstream):
    ...
    def build(self, rdata: Value, wdata: RegArray, SM_reg: RegArray, addr_reg: RegArray, we: RegArray, re: RegArray, radix_reg: RegArray, offset_reg: RegArray, mem_pingpong_reg: RegArray, mem_start: Value, mem_end: Value):
        ...
        with Condition(SM_reg[0] == UInt(2)(3)):
            ...
            with Condition(SM_MemImpl[0] == UInt(2)(2)):
                ...
                write_addr_reg[0] = radix_reg[idx][0:(addr_width-1)].bitcast(UInt(addr_width))  - UInt(addr_width)(1) + UInt(addr_width)(data_depth) - mem_start.bitcast(UInt(addr_width))
                ...
                with Condition(stop_reg[0] == UInt(1)(1)): # Stop
                    ...
                    addr_reg[0] = radix_reg[idx][0:(addr_width-1)].bitcast(UInt(addr_width))  - UInt(addr_width)(1) + UInt(addr_width)(data_depth) - mem_start.bitcast(UInt(addr_width))

            # Stage 3: Reset
            with Condition(SM_MemImpl[0] == UInt(2)(3)):
                with Condition(reset_cycle_reg[0] == UInt(4)(0)):
                    log("Stage 3-3: Writing wdata ({:08x}) to mem_addr ({});",wdata[0], addr_reg[0]) # Place holder to use read_cond for upstreams
                    re[0] = Bits(1)(0)
                    we[0] = Bits(1)(0)
                ...
                with Condition(reset_cycle_reg[0] == UInt(4)(15)):
                    log("Stage 3-3: Reset radix_reg[{}] to {:08x}.",reset_cycle_reg[0], UInt(data_width)(0))
                    log("Stage 3-3: Reset other registers: reset_cycle_reg[0]=0; SM_MemImpl[0]=0; SM_reg[0]=0; read_addr_reg[0]=0; write_addr_reg[0]=data_depth; stop_reg[0]=0;")
                    radix_reg[reset_cycle_reg[0]] = UInt(data_width)(0)
...
class Driver(Module):
    ...
    @module.combinational
    def build(self, memory_user: Module, radix_reducer: Module, cycle_reg: RegArray, radix_reg: RegArray, SM_reg: RegArray, addr_reg: RegArray, we: RegArray, re: RegArray, wdata: RegArray, offset_reg: RegArray, mem_pingpong_reg: RegArray):
        read_cond = ((mem_pingpong_reg[0] == UInt(1)(0)) & (addr_reg[0] < UInt(addr_width)(data_depth))) | ((mem_pingpong_reg[0] == UInt(1)(1)) & (addr_reg[0] < UInt(addr_width)(2*data_depth)))
        # Build Memory
...
def build_system():
    ...
    with sys:
        ...
        rdata = memory_user.build(SM_reg=SM_reg, radix_reg=radix_reg, offset_reg=offset_reg, addr_reg=addr_reg, mem_pingpong_reg=mem_pingpong_reg)
        # Create Radix Reducer
        radix_reducer = RadixReducer(width=data_width)
        radix_reducer.build(radix_reg, cycle_reg=cycle_reg)
        # Create driver
        driver = Driver()
        mem_start, mem_end = driver.build(memory_user, radix_reducer, cycle_reg=cycle_reg, radix_reg=radix_reg, SM_reg=SM_reg, addr_reg=addr_reg, we=we, re=re, wdata=wdata, offset_reg=offset_reg, mem_pingpong_reg=mem_pingpong_reg)
        # Create Memory Implementation
        mem_impl = MemImpl()
        mem_impl.build(rdata=rdata, wdata=wdata, SM_reg=SM_reg, addr_reg=addr_reg, we=we, re=re, radix_reg=radix_reg, offset_reg=offset_reg, mem_pingpong_reg=mem_pingpong_reg, mem_start=mem_start, mem_end=mem_end)
