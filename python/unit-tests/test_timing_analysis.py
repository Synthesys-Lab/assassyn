...
from assassyn.analysis.timing import critical_paths
...
from assassyn.ir.expr.arith import BinaryOp
...
def _node_kinds(path):
    """Return the timing-role labels for every node on a path."""

    return [node.kind for node in path.nodes]
...
def test_register_boundary_fixture_has_zero_delay_boundaries():
    ...
    assert _node_kinds(path) == ["regarray_read", "regarray_write"]


def test_fifo_boundary_fixture_has_fifo_source_and_sink():
    """FIFO pops should start paths and FIFO pushes should terminate them."""

    [path] = critical_paths(_build_fifo_boundary_system(), top_n=1)
