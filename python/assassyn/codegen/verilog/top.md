...
- **FIFO Depth Configuration**: Determines FIFO depths from `module_metadata.fifo.pushes` (no expression walking, predicate context preserved for downstream analysis, and the data is shared with the FIFO-indexed registry produced by the pre-pass)
...
- Metadata-driven checks for `FIFOPop` readiness: the predicated pop entries in `module_metadata.fifo.pops` (which are direct references to the registry-owned interactions emitted by the pre-pass) determine whether `<port>_pop_ready` connections should be emitted, replacing the legacy dumper helper traversal.
