...
- `array_metadata`: `ArrayMetadataRegistry` instance with write/read port assignments and user membership. Arrays whose owner is a memory instance and that match the memory's `_payload` are skipped during collection because they are handled by dedicated memory generators.
