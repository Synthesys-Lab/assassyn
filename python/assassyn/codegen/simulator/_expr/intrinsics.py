...
def _codegen_fifo_peek(node, module_ctx, **_kwargs):
...
def _codegen_fifo_valid(node, module_ctx, **_kwargs):
    """Generate code for FIFO_VALID intrinsic."""
    port_self = dump_rval_ref(module_ctx, node.get_operand(0))
    return f"!sim.{port_self}.is_empty()"


def _codegen_value_valid(node, module_ctx, **_kwargs):
...
def _codegen_module_triggered(node, module_ctx, **_kwargs):
    """Generate code for MODULE_TRIGGERED intrinsic."""
    port_self = dump_rval_ref(module_ctx, node.get_operand(0))
    return f"sim.{port_self}_triggered"


def _codegen_has_mem_resp(node, module_ctx, **_kwargs):
...
def _codegen_get_mem_resp(node, module_ctx, **_kwargs):
...
def _codegen_send_write_request(node, module_ctx, **_kwargs):
...
def codegen_intrinsic(node: Intrinsic, module_ctx, **kwargs):
