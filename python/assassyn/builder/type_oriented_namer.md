...
>>>>>>> 2af84c7 (wip: builder is nearly fully documented)
...
**Returns**: A sanitized string prefix that describes the node's type and operation
...
**Returns**: A sanitized string suitable for use as an identifier
...
### `_safe_getattr(node: Any, attr: str) -> Optional[Any]`
...
**Returns**: The attribute value or `None` if not found or if an error occurs
...
### `_entity_name(self, entity: Any) -> Optional[str]`
...
**Returns**: A sanitized name string or `None` if no name is found
...
### `_module_prefix(self, node: Any) -> str`
...
**Returns**: A string prefix with "Instance" suffix

**Explanation**: This method takes the class name of the node, sanitizes it, and appends "Instance" to create module instance names. It handles special cases where the base name is 'module' or 'modulebase' by normalizing to 'module'.

### `_unwrap_operand(self, entity: Any) -> Any`
...
**Returns**: The unwrapped entity or the original entity if unwrapping fails
...
### `_describe_operand(self, operand: Any) -> Optional[str]`
...
**Returns**: A descriptive token or `None` if no name is found
...
### `_combine_parts(self, *parts: Optional[str]) -> Optional[str]`
...
**Returns**: A combined identifier string or `None` if no valid parts are provided
...
### `_head_token_segment(token: str) -> str`
...
**Returns**: The first two segments of the token, joined by underscores
...
Module instances follow PascalCase naming with "Instance" suffix to ensure compatibility with Rust naming conventions and eliminate compiler warnings. For example:
