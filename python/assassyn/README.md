...
- `builder/`: The AST builder utilities, including the singleton AST context, and the decorators.
...
- `ramulator2/`: Optional Ramulator2 bindings that are loaded only when explicitly imported or accessed.

Ideally, each `*.py` file shall have a corresponding `*.md` file that documents the exposed interface
and internal implementation details. Prefer reading that markdown file when developing.

> NOTE: This is still a work in progress, and the documentation is not complete yet.
